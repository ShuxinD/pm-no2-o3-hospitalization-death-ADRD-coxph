...
dir_out <- "/nfs/home/S/shd968/shared_space/ci3_shd968/medicareADRD/github_repo/results/main_analysis/2index_event_bias/coxph_ReAd/"
pollutants <- c("pm25", "no2", "ozone_summer", "ox")

for (pollutants_i in pollutants){
  cat("preview the ipws for coxph model", pollutants_i, "\n")
...
temp_all3_raw <- dt[, deadipw_all3]*ipw_ieb_all3_raw
summary(temp_all3_raw)
summary(truncate_ipw(temp_all3_raw, 0.97, 0.03))
temp_all3 <- truncate_ipw(ipw_ieb_all3_raw, 0.97, 0.03)
...
temp_all3_raw <- dt[, deadipw_all2]*ipw_ieb_all2_raw
summary(temp_all2_raw)
summary(truncate_ipw(temp_all2_raw, 0.985, 0.015))
temp_all2 <- truncate_ipw(temp_all2_raw, 0.985, 0.015)
