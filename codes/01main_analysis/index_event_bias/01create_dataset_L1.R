...
colnames(dt_full)
temp <- colnames(dt_full)[3:21] 
colnames(dt_full)[3:21] <- paste0(temp, "_E0")
...
for (pollutants_i in pollutants) {
  ...
  if (pollutants_i=="no2"){
    mod <- lm(get(pollutants_i) ~ get(paste0(pollutants_i, "_E0")) + 
                mean_bmi_E0 + smoke_rate_E0 + 
                hispanic_E0 + pct_blk_E0 + medhouseholdincome_E0 + medianhousevalue_E0 + poverty_E0 + education_E0 + popdensity_E0 + pct_owner_occ_E0 +
                summer_tmmx_E0 + winter_tmmx_E0 + summer_rmax_E0 + winter_rmax_E0, 
              data = dt_mortality)
    # pdf(file="/nfs/home/S/shd968/shared_space/ci3_shd968/dementia/no2_A1L1.pdf")
    # par(mfrow=c(1,1))
    # plot(mod$fitted.values, mod$residuals)
    # dev.off()
    # summary(mod)$r.squared # 0.7910301
    # plot(mod)
  } else{
    mod <- lm(get(pollutants_i) ~ get(paste0(pollutants_i, "_E0")),
              data = dt_mortality)
    # summary(mod)$r.squared # 0.7864904
  }
  # mod <- lm(log(get(pollutants_i)) ~ log(get(paste0(pollutants_i, "_E0"))),
  #           data = dt_mortality)
  # # pdf(file="/nfs/home/S/shd968/shared_space/ci3_shd968/dementia/logno2_A1.pdf")
  # # par(mfrow=c(1,1))
  # # plot(mod$fitted.values, mod$residuals)
  # # dev.off()
  # summary(mod)$r.squared # 0.7567647
  # 
  # mod <- lm(log(get(pollutants_i)) ~ log(get(paste0(pollutants_i, "_E0"))) + 
  #             mean_bmi_E0 + smoke_rate_E0 + 
  #             hispanic_E0 + pct_blk_E0 + medhouseholdincome_E0 + medianhousevalue_E0 + poverty_E0 + education_E0 + popdensity_E0 + pct_owner_occ_E0 +
  #             summer_tmmx_E0 + winter_tmmx_E0 + summer_rmax_E0 + winter_rmax_E0, 
  #           data = dt_mortality)
  # # pdf(file="/nfs/home/S/shd968/shared_space/ci3_shd968/dementia/logno2_A1L1.pdf")
  # # par(mfrow=c(1,1))
  # # plot(mod$fitted.values, mod$residuals)
  # # dev.off()
  # summary(mod)$r.squared # 0.763916
  # # plot(mod)
  ...
  temp <- dt_mortality[,get(pollutants_i)]
  num <- dnorm(temp, mean(temp), sd(temp))
  # num <- 1/sqrt(2*pi*sd(temp)^2)*exp(-(temp-mean(temp))^2/(2*sd(temp)^2))
  gc()
  # conditional density (GPS)
  # denom <- dnorm(dt_mortality[,get(pollutants_i)], mod$fitted.values, summary(mod)$sigma)
  denom <- dnorm(temp, mean=mod$fitted.values, sd=summary(mod)$sigma)
  gc()
  ipw_ieb <- num/denom
  # hist(ipw_ieb)
  # summary(ipw_ieb)
  # summary(truncate_ipw(ipw_ieb, 0.975, 0.025))
...
for (pollutants_i in pollutants) {
  ...
  cat("FINISHING PS model\n")
  temp <- dt_ReAd[,get(pollutants_i)]
  # marginal density
  num <- dnorm(temp, mean(temp), sd(temp))
  gc()
  # conditional density (GPS)
  denom <- dnorm(temp, mod$fitted.values, summary(mod)$sigma)
  gc()
  ipw_ieb <- num/denom
  assign(paste0("ipw_ieb_", pollutants_i), ipw_ieb)
}
