...
The dataset `ADRDpeople.csv` was directly generated from the denominator files, which follow the ADRD people till their death or the end of 2016.
...
- have follow-up info for every year after the year they were hospitalized with ADRD for the first time (`firstADRDyr`) till the end of study period (2016) or death or leaving the US, whichever comes first.
...
We constructed a temporary dataset named `temp` which is a subset of `ADRDcohort` after removing NA (natually had removed rows whose `year==firstADRDyr` when merging with exposures), and summarize each person as one row with `r dim(sum_temp)[1]` subjects:
- generated `start_yr` as the minimum of calendar year
...
```{r echo=FALSE}
cat("the number of subjects in ADRDpeople(removed NAs) is", dim(sum_temp)[1], "\n")
cat("the number of person-years in ADRDpeople(removed NAs) is", dim(temp)[1], "\n")
...
```{r echo=FALSE}
temp[qid %in% sum_temp[start_yr != (firstADRDyr+1), qid], ]
```

```{r echo=FALSE}
cat("the number of person-years of related subjects is", dim(temp[qid %in% sum_temp[start_yr != (firstADRDyr+1), qid], ])[1], "\n")
```
...
The above subject (`r dim(sum_temp[(end_yr-firstADRDyr) != count,])[1]` subjects in total) do not have each year's info during follow-up. **Consider deleting them**

Their info in denominator files:
```{r echo=FALSE}
temp[qid %in% sum_temp[(end_yr-start_yr+1) != count,][,qid],]
```
