...
dir_enrolledInfo <- file.path(getwd(), "codes", "revise_all_hospitalization", "All_EnrolledInfo.csv")
dir_denom <- file.path("/n/dominici_nsaph_l3/Lab/projects/analytic", "denom")
...
dir_output <- file.path(getwd(), "codes", "revise_all_hospitalization")
...
enrolledInfo <- fread(file.path(dir_enrolledInfo, "AllEnrolledInfo.csv"))
dim(enrolledInfo)[1]
...
uniqueN(ALLpeople[,qid]) # number of ADRD people in denominator file
