...
outcome <- "ReAd"
############################
dir_EMM_boots_data <- paste0("results/bootstrapping/EMM/", outcome, "/")
...
for (modifiers_i in modifiers) {
  for (pollutants_i in pollutants) {
    dt_raw <- read.csv(paste0(dir_EMM_boots_data, "cox_", outcome,"_EMM_",modifiers_i, "_", pollutants_i, "_HR.csv"))
    dt_i <- data.frame(modifier = modifiers_i,
                       pollutant = pollutants_i,
                       level = dt_raw$rn,
                       HR = dt_raw$HR,
                       HR_lci = dt_raw$HR_lci_bootstrap,
                       HR_uci = dt_raw$HR_uci_bootstrap)
    print(dt_i)
    dt <- rbind(dt,dt_i)
  }
}
...
dt$modifier <- factor(dt$modifier, levels = c("sex", "dual", "above_median_popdensity", "entry_age_over85", "race_collapsed"), 
                      labels = c("Sex", "Dual elegibility", "Above median population density", "Entry age over 85", "Race"))
dt$pollutant <- factor(dt$pollutant, levels = c("pm25", "no2", "ozone_summer", "ox"),
                       labels = c("PM[2.5]", "NO[2]", "Summer ozone", "Ox"))
View(dt)
fwrite(dt, paste0(dir_EMM_boots_data, "EMM_bootstrap_table_", outcome, ".csv"))
