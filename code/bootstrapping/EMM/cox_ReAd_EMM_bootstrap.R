...
dir_bootsdata <- paste0(getwd(),"/data/bootstrap/ReAd/")
dir_estimate0 <- paste0(getwd(),"/results/main_analysis/EMM/coxph_ReAd/")

dir_bootstrap_results <- paste0(getwd(), "/results/bootstrapping/EMM/ReAd/")

# num_uniq_zip <- 33608L # mortality
num_uniq_zip <- 33532L # ReAd
## load IQRs----
IQRs <- readRDS(paste0(dir_iqrdata,"IQRs_ReAd.rds"))
...
for (pollutants_i in pollutants) {
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_sex_", pollutants_i, ".csv"))
  ...
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_sex_", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping sex", pollutants_i, "\n")
}
...
for (pollutants_i in pollutants) {
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_dual_", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  var_boots_level0 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level0)
  var_boots_level1 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level1)
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_dual_", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping dual", pollutants_i, "\n")
}


## by population density (above median) ----
# median_popdensity # 1003.901
for (pollutants_i in pollutants) {
  cox_coefs_boots_level0<-NULL
  cox_coefs_boots_level1<-NULL
  for (boots_id in 1:100) {
    # set.seed(boots_id)
    dt_boots<- read_fst(paste0(dir_bootsdata, boots_id,".fst"), as.data.table = T)
    dt_boots[, above_median_popdensity := popdensity>1003.901]
    # model
    cox <- coxph(Surv(time = followupyr_start, time2 = followupyr_end, event = ReAd) ~ 
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_above_median_popdensity_", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  var_boots_level0 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level0)
  var_boots_level1 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level1)
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_above_median_popdensity_", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping above_median_popdensity", pollutants_i, "\n")
}
...
for (pollutants_i in pollutants) {
  ...
  for (boots_id in 1:100) {
    ...
    cox <- coxph(Surv(time = followupyr_start, time2 = followupyr_end, event = ReAd) ~ 
                   ...
                   summer_tmmx + winter_tmmx + summer_rmax + winter_rmax +
                   as.factor(year) +  as.factor(region) +
                   strata(as.factor(entry_age_break), as.factor(sex), as.factor(race_collapsed), as.factor(dual)), as.factor(dual)) + cluster(qid),
                 weights = get(paste0("deadipw_",pollutants_i)),
                 data = dt_boots,
                 tie = c("efron"),
                 na.action = na.omit)
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_entry_age_over85_", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  var_boots_level0 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level0)
  var_boots_level1 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level1)
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_entry_age_over85_", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping entry_age_over85", pollutants_i, "\n")
}
...
for (pollutants_i in pollutants) {
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_race_collapsed_", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_race_collapsed_", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping race_collapsed", pollutants_i, "\n")
}
## by poverty (above median) ----
# median_poverty # 0.08012821
for (pollutants_i in pollutants) {
  cox_coefs_boots_level0<-NULL
  cox_coefs_boots_level1<-NULL
  for (boots_id in 1:100) {
    # set.seed(boots_id)
    dt_boots<- read_fst(paste0(dir_bootsdata, boots_id,".fst"), as.data.table = T)
    dt_boots[, above_median_poverty := poverty > 0.08012821]
    # model
    cox <- coxph(Surv(time = followupyr_start, time2 = followupyr_end, event = ReAd) ~ 
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_above_median_poverty_", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  var_boots_level0 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level0)
  var_boots_level1 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level1)
  ...
  HR_results <- as.data.frame(HR_results)
  rownames(HR_results) <- paste0("level", levels(dt_boots[,as.factor(above_median_poverty)]))
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_above_median_poverty_", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping above_median_poverty", pollutants_i, "\n")
}

## by dual==1 race_collapsed ----
for (pollutants_i in pollutants) {
  cox_coefs_boots_level0<-NULL
  cox_coefs_boots_level1<-NULL
  cox_coefs_boots_level2<-NULL
  ...
  for (boots_id in 1:100) {
    # set.seed(boots_id)
    dt_boots<- read_fst(paste0(dir_bootsdata, boots_id,".fst"), as.data.table = T)[dual==1, ]
    # model
    cox <- coxph(Surv(time = followupyr_start, time2 = followupyr_end, event = ReAd) ~ 
                   get(pollutants_i)*as.factor(race_collapsed) + 
                   mean_bmi + smoke_rate + 
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_race_collapsed_dual1", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  coef0_level2 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-1,coef]
  coef0_level3 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-2,coef]
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_race_collapsed_dual1", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping race_collapsed", pollutants_i, "\n")
}

## by dual==0 race_collapsed ----
for (pollutants_i in pollutants) {
  cox_coefs_boots_level0<-NULL
  cox_coefs_boots_level1<-NULL
  cox_coefs_boots_level2<-NULL
  ...
  for (boots_id in 1:100) {
    # set.seed(boots_id)
    dt_boots<- read_fst(paste0(dir_bootsdata, boots_id,".fst"), as.data.table = T)[dual==0, ]
    # model
    cox <- coxph(Surv(time = followupyr_start, time2 = followupyr_end, event = ReAd) ~ 
                   get(pollutants_i)*as.factor(race_collapsed) + 
                   mean_bmi + smoke_rate + 
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_EMM_race_collapsed_dual0", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  coef0_level2 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-1,coef]
  coef0_level3 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-2,coef]
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_ReAd_EMM_race_collapsed_dual0", pollutants_i, "_HR.csv"))
  cat("save HR for cox ReAd EMM bootstrapping race_collapsed", pollutants_i, "\n")
}