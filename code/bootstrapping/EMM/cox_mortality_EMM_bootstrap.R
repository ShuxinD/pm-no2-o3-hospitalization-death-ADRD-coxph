...
for (pollutants_i in pollutants) {
  ...
  for (boots_id in 1:100) {
    # set.seed(boots_id)
    dt_boots<- read_fst(paste0(dir_bootsdata, boots_id,".fst"), as.data.table = T)
    dt_boots[, above_median_popdensity := popdensity>1066.189]
    # model
    cox <- coxph(Surv(time = followupyr_start, time2 = followupyr_end, event = dead) ~ 
...
for (pollutants_i in pollutants) {
  ...
  for (boots_id in 1:100) {
    # set.seed(boots_id)
    dt_boots<- read_fst(paste0(dir_bootsdata, boots_id,".fst"), as.data.table = T)
    dt_boots[, above_median_poverty := poverty > 0.08114468]
    # model
    cox <- coxph(Surv(time = followupyr_start, time2 = followupyr_end, event = dead) ~ 
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_mortality_EMM_above_median_poverty_", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  var_boots_level0 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level0)
  var_boots_level1 <- floor((num_uniq_zip)^0.7)/num_uniq_zip*var(cox_coefs_boots_level1)
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_mortality_EMM_above_median_poverty_", pollutants_i, "_HR.csv"))
  cat("save HR for cox mortality EMM bootstrapping above_median_poverty", pollutants_i, "\n")
}
...
for (pollutants_i in pollutants) {
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_mortality_EMM_race_collapsed_dual1", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  coef0_level2 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-1,coef]
  coef0_level3 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-2,coef]
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_mortality_EMM_race_collapsed_dual1", pollutants_i, "_HR.csv"))
  cat("save HR for cox mortality EMM bootstrapping race_collapsed", pollutants_i, "\n")
}
...
for (pollutants_i in pollutants) {
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_mortality_EMM_race_collapsed_dual0", pollutants_i, ".csv"))
  coef0_level0 <- estimate0[1,coef]
  coef0_level1 <- estimate0[1,coef] + estimate0[dim(estimate0)[1],coef]
  coef0_level2 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-1,coef]
  coef0_level3 <- estimate0[1,coef] + estimate0[dim(estimate0)[1]-2,coef]
  ...
  setDT(HR_results, keep.rownames = T)
  print(HR_results)
  fwrite(HR_results, paste0(dir_bootstrap_results, "cox_mortality_EMM_race_collapsed_dual0", pollutants_i, "_HR.csv"))
  cat("save HR for cox mortality EMM bootstrapping race_collapsed", pollutants_i, "\n")
}