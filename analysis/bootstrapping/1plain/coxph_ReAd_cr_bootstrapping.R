...
dir_deadipw <- paste0(getwd(),"/data/")
dir_bootsdata <- paste0(getwd(),"/data/bootstrap/ReAd/")
dir_estimate0 <- paste0(getwd(),"/results/main_analysis/1plain/coxph_ReAd_cr/")
...
dt_deadipw <- read_fst(paste0(dir_deadipw,"ADRDcohort_ReAd_deadipw.fst"), as.data.table = T)
...
for (pollutants_i in pollutants) {
  ...
  for (boots_id in 1:100) {
    ...
    dt_boots <- merge.data.table(dt_boots, dt_deadipw[,.(qid,year,get(paste0("deadipw_", pollutants_i)))])
    ## single pollutant model
  ...
  estimate0 <- fread(paste0(dir_estimate0, "cox_ReAd_", pollutants_i, ".csv"))
  ...
  fwrite(HR_results, paste0(dir_bootstrap_results, "ReAd_", pollutants_i, ".csv"))
}
rm(estimate0)
rm(coef0)
rm(var_boots)
...
fwrite(HR_results, paste0(dir_bootstrap_results, "ReAd_all3.csv"))
...
fwrite(HR_results, paste0(dir_bootstrap_results, "ReAd_all2.csv"))