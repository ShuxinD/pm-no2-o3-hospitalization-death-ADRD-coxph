...
for (pollutants_i in pollutants){
  ...
  fwrite(tb, file.path(rundir, "coxph_ReAd", paste0("cox_ReAd_", pollutants_i, ".csv")))
  cat("output coefs...\n")
  HR <- tb[1,]
  ...
  print(HR)
  fwrite(HR, file.path(rundir, "coxph_ReAd", paste0("cox_ReAd_", pollutants_i, "_HR.csv")))
...
tb <- as.data.frame(tb)
setDT(tb, keep.rownames = TRUE)[]
fwrite(tb, file.path(rundir, "coxph_ReAd", "cox_ReAd_all3.csv"))
...
fwrite(HR, file.path(rundir, "coxph_ReAd", "cox_ReAd_all3_HR.csv"))
...
tb <- as.data.frame(tb)
setDT(tb, keep.rownames = TRUE)[]
fwrite(tb, file.path(rundir, "coxph_ReAd", "cox_ReAd_all2.csv"))
...
HR[, `:=`(HR_IQR = exp(coef*IQRunit),
          HR_lci = exp((coef-1.96*`robust se`)*IQRunit),
          HR_uci = exp((coef+1.96*`robust se`)*IQRunit))][]
fwrite(HR, file.path(rundir, "coxph_ReAd", "cox_ReAd_all2_HR.csv"))
